...
class PointsClassification:
    def __init__(self):
        ...
        self.canvas.create_line(0,250,500,250,tags="horizontal",fill="gray",dash=(5,5))
        self.canvas.create_line(250,0,250,500,tags="vertical",fill="gray",dash=(5,5))
        self.canvas.pack()
        ...
        self.greenPoints=tk.Radiobutton(frame_bt,text = "blue",foreground="blue", variable=self.pointsLabel,value=1,state="disabled",command=self.test)
        self.redPoints=tk.Radiobutton(frame_bt,text="red",foreground="red",variable=self.pointsLabel,value=-1,state="disabled",command=self.test)
        ...
        try:
            ...
            if self.pointingStatue:
                if self.pointsLabel.get() == 1:
                    print("label = blue")
                    self.canvas.create_text(event.x,event.y,text='*',fill="blue")
                    self.canvas.pack()
                    self.blueSet.append([event.x, event.y,self.pointsLabel.get()])
                elif self.pointsLabel.get() == -1:
                    print("label = red")
                    self.canvas.create_text(event.x,event.y,text='*',fill="red")
                    self.canvas.pack()
                    self.greenSet.append([event.x, event.y,self.pointsLabel.get()])
        except:
            pass
